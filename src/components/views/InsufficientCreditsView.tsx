...
export function InsufficientCreditsView({ onBack, onGoToStore }: InsufficientCreditsViewProps) {
  return (
    ...
    >
      {/* Background Orbs */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-red/10 blur-[120px] rounded-full -translate-y-1/2 translate-x-1/2" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-brand-orange/10 blur-[100px] rounded-full translate-y-1/2 -translate-x-1/2" />

      <div className="max-w-4xl w-full text-center relative z-10">
        <div className="relative inline-block mb-8">
          <div className="absolute inset-0 bg-brand-red/40 blur-3xl rounded-full animate-pulse" />
          <div className="relative w-20 h-20 rounded-[28px] bg-brand-gradient flex items-center justify-center text-white shadow-2xl shadow-brand-red/30">
            <Sparkles size={40} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <motion.div
            whileHover={{ y: -8 }}
            className="p-8 rounded-[32px] bg-white/5 border border-white/10 hover:border-brand-red/50 transition-all backdrop-blur-sm group relative overflow-hidden"
          >
            <div className="w-12 h-12 rounded-2xl bg-white/5 text-white flex items-center justify-center mb-6 group-hover:bg-brand-red transition-all">
              <Rocket size={24} />
            </div>
          ...
          <motion.div
            whileHover={{ y: -8 }}
            className="p-8 rounded-[32px] bg-brand-surface border-2 border-brand-red relative shadow-2xl group overflow-hidden"
          >
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-brand-gradient text-white text-[10px] font-black rounded-full uppercase tracking-widest shadow-lg z-20">
              Mais Popular
            </div>
            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-red/10 blur-2xl rounded-full" />
            
            <div className="w-12 h-12 rounded-2xl bg-brand-red text-white flex items-center justify-center mb-6 shadow-lg shadow-brand-red/20">
              <Star size={24} />
            </div>
            ...
            <button 
              onClick={onGoToStore}
              className="w-full py-4 rounded-2xl bg-brand-gradient text-white font-bold text-sm shadow-xl shadow-brand-red/20 hover:brightness-110 transition-all"
            >
              Selecionar
            </button>
          </motion.div>
